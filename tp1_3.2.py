...
def insert_group_name(cursor, group_id, name):
    try:
        ...
        if existing_group:
            #print(f"Group '{name}' already exists with id {existing_group[0]}. Skipping insertion.")
...
def insert_category(cursor, id, category):
    try:
        ...
        if existing_category:
            #print(f"Category '{category}' already exists with id {existing_category[0]}. Skipping insertion.")
            pass
        else:
            try:
                cursor.execute('''
                ...
                ''', (id, category))
                conn.commit()  # Commit changes after insertion
                #print(f"Category inserted successfully! {id} {category}")
            except psycopg2.Error as e:
                conn.rollback()  # Rollback changes in case of an error
...
def insert_similar_product(cursor, product_A_asin, product_B_asin ):
    try:
        ...
        conn.commit()  # Commit changes after insertion
        #print("Similar Product inserted successfully!")
    except psycopg2.Error as e:
        conn.rollback()  # Rollback changes in case of an error
...
def insert_category_product(cursor, category_id, product_id):
    try:
        ...
        if existing_relation:
            #print(f"Relation with category_id {category_id} and product_id {product_id} already exists. Skipping insertion.")
            pass
        else:
            cursor.execute('''
            ...
            ''', (category_id, product_id))
            conn.commit()  # Commit changes after insertion
            #print("Category_Product inserted successfully!")
    except psycopg2.Error as e:
        conn.rollback()  # Rollback changes in case of an error
...
with open('amazon-meta.txt', 'r') as file:
    ...
    for line in file:
        ...
        if line.startswith('Id:'):
            ...
            comecou_reviews = Falsecomecou_reviews = False
